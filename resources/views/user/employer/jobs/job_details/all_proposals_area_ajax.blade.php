         ...
         @endphp
            ...
            <div class="allproposalLists_newcss">
               <div class="jobDetails">
                     <h4>{{ !empty($proposal->jobDetail->job_title) ? $proposal->jobDetail->job_title : 'No Title' }}</h4>
                     <p>{{ !empty($proposal->jobDetail->job_description) ? substr($proposal->jobDetail->job_description, 0, 70) . '...' : ''  }}
                      <br/>
                      <a
                        href="{{ !empty($proposal->jobDetail->id) ? route('user.job', ['id' => encryptUrlId($proposal->jobDetail->id) ]) : '' }}"
                        target="_blank">
                          View job posting
                      </a>
                     </p>
                    ...
                    <div class="actnBtns">
                      ...
                      onclick='getProposalModel("{{$proposal->id}}","{{ route('user.get_proposal_content', ['id' => $proposal->id ])}}");'
                        ...
                        @if(empty($proposal->myJobContract))
                           ...
                           >Hire</a>
                         @elseif(!empty($proposal->myJobContract) && !in_array($proposal->myJobContract->contract_status, array('1','3','4'))  )
                         <a class="btn btn-primary"
                            href="{{route('user.contract', ['id' => encryptUrlId($proposal->myJobContract->id) ])}}"
                ...
                <div class="modal-body text-center"  >
                  <input type="hidden" name="contractId" id="contractId" value=""/>
                   ...
                   <h1 class="modal-title">
                      Your application for the <b id="freelancerName">[{{ !empty($job->job_title) ? $job->job_title : 'No Title'}}]</b> is on its way!
                   </h1>
                </div>
