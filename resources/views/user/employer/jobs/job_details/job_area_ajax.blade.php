...
// die;?>
   <div class="left-right actve-drop-sec">
          <span class="25-jobs found-count" href="javascript:void(0);">
            {{ $jobs->total() }} {{ ($jobs->total() > 1) ? 'jobs' : 'job' }} found</span>
           <span class="span-main right-menu">
                Sort by:
              <div class="custom-selectbox">
                <select class="sortingSelect" name="sorting_on" onchange="applySorting();">
                  <option
                   value="posted_at"
                    ...
                    <h4 class="panel-title"
                      onclick='get_jobBasicF("{{$job->id}}","{{ route('user.get_jobBasic', ['id' => $job->id]) }}","alljobs","{{$job->job_status}}")' >
                       ...
                       aria-controls="collapseOne">
                          ...
                          {{ (trim($job->job_title) != '') ? $job->job_title : 'No Title'  }} -  {{ ($job->job_type == '1') ? "Hourly Rate" : "Project Base" }}
