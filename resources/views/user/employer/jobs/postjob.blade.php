               ...
               <div class="row">
                  <div class="col-sm-12">
                     <div class="post-suces-img text-center">
                         <img class="sucessful-img" src="{{ asset('../assets/img/successful.png') }}" alt="">
                     </div>
                  </div>
                  <div class="col-sm-12">
    ...
    var inQu_error = 0 ;
       $( ".interview_questions" ).each(function( index ) {
         if($.trim($(this).val()) == ''){
            ...
            $(this).parent().find('span.help-block').html('This is required!');
            $(this).parent().find('span.help-block').hide();
         }
      });
       ...
       if(inQu_error == 0){
           ...
           formData.append('skills' , ($('#skills').val() != null) ? $('#skills').val().join(',') : '');
           formData.append('expertise' , ($('#services').val() != null) ? $('#services').val().join(',') : '');
           formData.append('jobId', $('#jobId').val() );
           formData.append('uniq_job_id', $('#uniq_job_id').val());
...
function saveAndPublish(){
   ...
   if($("#job_type").val() === "1" && $("input[name='hourly_rate']").val() === ""){
      ...
      errors_all++;
   }else if( $("input[name='project_length']").val() === ""){
      errors_all++;
   }else if($("#job_type").val() === "2" && $("input[name='total_cost']").val() === ""){
      errors_all++;
   }else if($("textarea[name='job_title']").val() === "" || $("input[name='job_description']").val() === ""){
      errors_all++;
   ...
   if(errors_all == 0){
      var formData = new FormData($('form[name="jobpost_qualifications"]')[0]);
        formData.append('skills' ,($('#skills').val() != '') ? $('#skills').val().join(','): '');
        formData.append('expertise' ,($('#services').val() != '') ? $('#services').val().join(','): '');
        formData.append('jobId', $('#jobId').val() );
        formData.append('uniq_job_id', $('#uniq_job_id').val());
...
function saveAndExit(){
   //job title is required to save and exit
   if($.trim($('textarea[name="job_title"]').val()) != '') {
      showScreenLoader();
      var formData = new FormData($('form[name="jobpost_qualifications"]')[0]);
      ...
      $.toast({
          heading: 'Warning!',
          text: "Job name is required to save the job as draft.",
          showHideTransition: 'slide',
          icon: 'warning'
      });
      if($('#headingOne').find('button').hasClass('collapsed')) {
