...
function getMilestoneNames($ids){
  ...
  for($i=0; $i<count($ids); $i++){
    try{
      $milestone = JobContractMilestones::find($ids[$i]);
      $milestone_names[] = !empty($milestone->milestone) ?  $milestone->milestone : 'Milestone Not exist ('  .$ids[$i]. ')';
    }catch(ModelNotFoundException $e){ 
      $milestone_names[] = 'Milestone Not exist ('  .$ids[$i]. ')';
    }
...
function getTimesheetNames($ids){
  ...
  for($i=0; $i<count($ids); $i++){
   try{ 
    $timesheet = JobContractTimesheet::find($ids[$i]);
    $timesheets_names[] = !empty($timesheet->description) ?  $timesheet->description : 'Timesheet Not exist ('  .$ids[$i]. ')';
   }catch(ModelNotFoundException $e){
     $timesheets_names[] = 'Timesheet Not exist ('  .$ids[$i]. ')';
   }
...
function getDateAgo($date, $type = 'posted'){
    ...
    if($starttime != $datetime)
    {
        ...
        if($years != 0 || $months != 0) 
        { 
          $timeago = date("M d, Y", strtotime($datetime));
