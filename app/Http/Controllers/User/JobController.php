       ...
       if($_POST)
         {
            ...
            $input = $request->all();
            if(!empty($input['hourly_rate']) || !empty($input['weekly_limit']) || !empty($input['project_length']) || !empty($input['total_cost']) ||!empty(trim(@$input['job_title'])) || !empty($input['job_description']) || !empty($input['expertise']) || !empty($input['skills']) || !empty($input['english_prof'])  || !empty($input['id']))
            {
               // echo "<pre>";
                //print_r($_POST);
