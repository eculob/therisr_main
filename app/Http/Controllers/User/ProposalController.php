...
class ProposalController extends Controller
{
    ...
    function get_proposals_area_ajax($page,$jid){ 
        ...
        if($page == 'liked'){
           $data['proposals'] = JobProposals::where(array(
                                'like_status'=>'2',
                                'job_id' => $jid
                             ))->get();
        }else if($page == 'archived'){
           $data['proposals'] = JobProposals::onlyTrashed()->where(array(
                                  'job_id' => $jid
                                 ))->get();
        }else{
          $data['proposals'] = JobProposals::where(array(
    ...
    function get_alljobs_proposals_area_ajax($page){  
        if($page == 'liked'){
           $data['proposals'] = JobProposals::where(array(
                                ...
                                ->whereIn('id', JobContract::select('proposal_id')
                                      ->where('contract_status','!=','3') // means hire archived nhi hai
                                      ->where(array('user_by' => Auth::user()->id))
                                      ->get()->toArray()
                                )->paginate($this->per_page_data);
        }else if($page == 'archived'){
           $data['proposals'] = JobProposals::onlyTrashed()
                                ->whereIn('id', JobContract::select('proposal_id')
                                      ->where('contract_status','!=','3') // means hire archived nhi hai
                                      ->where(array('user_by' => Auth::user()->id))
                                      ->get()->toArray()
    ...
    function get_proposal_content($id){
    	 ...
    	 $data['job'] = Jobs::withTrashed()->find($proposal->job_id);
